...
export default class Equals extends Rete.Component {
	...
	async builder(node){
		...
		node
		.addInput(
			new Rete.Input("num1", "Value", Sockets.AnyValue ),
		)
		.addInput(
			new Rete.Input("num2", "Value", Sockets.AnyValue )
		)
		.addOutput(
	...
	async worker(node, inputs, outputs){
		...
		const evaluate = (val1, val2) => {
			// console.log(inputs);
			if (inputs.num1 && inputs.num1[0] && inputs.num1[0].num){
			...
			else if (inputs.num1 && inputs.num1[0] && inputs.num1[0].string){
				this.valueTable[node.id].num1 = inputs.num1 && inputs.num1[0] ? inputs.num1[0].string : 0;
				this.valueTable[node.id].num2 = inputs.num2 && inputs.num2[0] ? inputs.num2[0].string : 0;
			}
